    ...
    for i in match_details_teams:
        ...
        if match_details_team:
            if match_details_team.find('span'):
                ...
                part_teams_string = str(match_details_team.find('span').text.split(',')[0])
                part_teams = sanitize_team(part_teams_string).split(' vs ')

                if i.find('a', {'class': 'cb-text-complete'}):
                    token_string = str(i.find('a', {'class': 'cb-text-complete'}).text)
                    # Winner team is taken from elapsed matches
                    status = [str(sanitize_team(token_string.split(' won by')[0]))]
                else:
                    # Both teams are part of upcoming matches
                    status = part_teams

                # Handle 'No-Result' case
                if ("abandoned" in status[0] or
                        "No result" in status[0]):
                    status = part_teams
                    status.extend(["NR"])

                match_result[part_teams_string] = status
...
def scrap_tournament_odds():
    ...
    for match in match_list:
        if len(match) == 2:
            team_a = full_name(match[0])
            team_b = full_name(match[1])
            odds.append(func_lst_ten_mtchs(team_a, team_b))
        else:
            odds.append([100])
...
def func_lst_ten_mtchs(teama, teamb):
    ...
    for i_yr in range(0, 10):
        ...
        yr = 2019 - i_yr
        main_base_url = 'http://stats.espncricinfo.com/ci/engine/records/team/match_results.html?class=2;id=' + str(
            yr) + ';team=' + team_id + ';type=year'
        ...
        for mtch_date_lp in mtch_dtls_full_desc:
            ...
            for mtch_date_lp_2 in mtch_date_lp:
                if ('Jan') in str(mtch_date_lp_2.text) or ('Feb') in str(mtch_date_lp_2.text) or ('Mar') in str(
                        mtch_date_lp_2.text) or ('Apr') in str(mtch_date_lp_2.text) or ('May') in str(
                    mtch_date_lp_2.text) or ('Jun') in str(mtch_date_lp_2.text) or ('Jul') in str(
                    mtch_date_lp_2.text) or ('Aug') in str(mtch_date_lp_2.text) or ('Sep') in str(
                    mtch_date_lp_2.text) or ('Oct') in str(mtch_date_lp_2.text) or ('Nov') in str(
                    mtch_date_lp_2.text) or ('Dec') in str(mtch_date_lp_2.text):
                    ...
                    if len(str(mtch_date_lp_2.text).split(',')[0]) == 5:
                        ...
                        dt_value = str(mtch_date_lp_2.text).split(',')[0][0:6].replace('-', '') + ',' + \
                                   str(mtch_date_lp_2.text).split(',')[1]
        ...
        for k in range(0, len(main_lst)):
            if (((main_lst[k][0] == teama and main_lst[k][1] == teamb) or (
                    main_lst[k][1] == teama and main_lst[k][0] == teamb)) and str(main_lst[k][2]) != 'tied'):
    ...
    if len(last_10_macthes) != 0:
        return [(float(teama_val) / float(len(last_10_macthes))) * 100,
                (float(teamb_val) / float(len(last_10_macthes))) * 100]
    else:
        return [50, 50]
